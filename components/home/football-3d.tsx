...
/**
 * Interactive 3D "match orb" — a faceted geodesic football in gold-on-black.
...
export function Football3D({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    // Lighting — gold key + fill for the metallic look
    ...
    const animate = () => {
      ...
      // ease toward cursor lean on top of the spin
      group.rotation.x += (target.x - group.rotation.x * 0.02) * 0.02;
  ...
  return (
    <div
      ref={containerRef}
      aria-hidden
      className={cn("h-full w-full cursor-grab", className)}
    />
  );
}
