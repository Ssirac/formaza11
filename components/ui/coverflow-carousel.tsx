...
export interface CoverflowCarouselProps {
  ...
  /** Advance automatically. Pauses while dragging or hovering. */
  autoPlay?: boolean;
  /** Milliseconds between auto-advances. @default 3000 */
  autoPlayInterval?: number;
  /** Fires with the centred slide's index whenever it changes. */
...
export function CoverflowCarousel({
  ...
  /** True while the user is dragging or hovering — auto-play holds off. */
  ...
  // Auto-advance. Held off while the pointer is down or hovering, and skipped
  // entirely for viewers who ask for reduced motion.
  React.useEffect(() => {
    ...
    const id = window.setInterval(() => {
      if (!heldRef.current) nudge(1);
    }, Math.max(600, autoPlayInterval));
    return () => window.clearInterval(id);
  }, [autoPlay, autoPlayInterval, count, nudge]);

  // Surface the centred slide so a parent can react (e.g. link to it).
  React.useEffect(() => {
    onSelect?.(selected);
  }, [selected, onSelect]);
    ...
    >
      <div className="relative">
        <div
          ...
          onPointerLeave={() => {
            if (!dragRef.current) heldRef.current = false;
          }}
          onKeyDown={(event) => {
          ...
          className="cursor-grab overflow-hidden py-10 outline-none ring-gold focus-visible:ring-2 active:cursor-grabbing"
          style={{
          ...
          >
            {slides.map((slide, index) => (
              <div
                ...
                className={cn(
                  "absolute left-1/2 top-0 aspect-square overflow-hidden rounded-2xl bg-ink-2 shadow-xl will-change-transform",
                  cardClassName,
                )}
        ...
        {showNavigation && (
          <>
            <button
              ...
              onClick={() => nudge(-1)}
              className="absolute left-3 top-1/2 z-[200] -translate-y-1/2 rounded-full border border-line-strong bg-black/60 p-2 text-cream backdrop-blur transition hover:bg-black/80"
            >
            ...
            <button
              ...
              onClick={() => nudge(1)}
              className="absolute right-3 top-1/2 z-[200] -translate-y-1/2 rounded-full border border-line-strong bg-black/60 p-2 text-cream backdrop-blur transition hover:bg-black/80"
            >
        ...
        <div className="mt-6 flex items-center justify-center gap-2">
          {slides.map((_, index) => (
            <button
              ...
              onClick={() => goTo(index)}
              className={cn(
                "size-2 rounded-full bg-gold transition-opacity",
